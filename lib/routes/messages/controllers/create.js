...
module.exports = async(req, res) => {
    ...
    jwt.verify(authorization, process.env.JWT, async(err, decoded) => {
        ...
        if (!await user.findById(String(to))) return res.status(404).json(Response.notfound)

        if (String(to) === String(decoded.id)) return res.status(400).json(new Response(400, `sender id: ${decoded.id}, user id: ${to}`))

        message.create({
            _id: flake.gen(),
            author: String(decoded.id),
            to: String(to),
            content: content,
            CreatedAt: Date.now()
        }).then((a) => {
